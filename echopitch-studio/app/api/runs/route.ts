...
export async function POST(request: Request) {
  try {
    ...
    if (!pitchDurations.includes(targetDuration as (typeof pitchDurations)[number])) return NextResponse.json({ error: "Duration must be 30, 60, or 90 seconds." }, { status: 400 });
