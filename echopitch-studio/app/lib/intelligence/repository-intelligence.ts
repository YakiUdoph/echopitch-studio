...
const signalRules: SignalRule[] = [
  ...
  { id: "http-api", name: "HTTP API", description: "Exposes request handlers for programmatic use.", filePattern: /(?:api|route|handler|server|controller)/i, contentPattern: /(?:export async function (?:GET|POST|PUT|PATCH|DELETE)|app\.(?:get|post|put|patch|delete)\(|router\.(?:get|post)|http\.HandleFunc|@app\.(?:get|post))/i, reason: "An HTTP request handler is implemented in source code." },
  { id: "http-client", name: "Fetch-based HTTP client", description: "Sends HTTP requests through the Fetch API.", contentPattern: /(?:\bfetch\s*\(|\bnew\s+Request\s*\()/i, reason: "Source code invokes Fetch or constructs web Request objects." },
  ...
  { id: "typed-models", name: "Typed domain models", description: "Uses explicit types or schemas for application data.", filePattern: /(?:types?|schema|\.prisma|\.graphql|\.proto)/i, contentPattern: /(?:export interface|export type|type Query|model \w+|message \w+)/i, reason: "Repository defines explicit domain types or schemas.", mechanism: true },
  { id: "automated-tests", name: "Automated tests", description: "Includes executable automated test coverage.", filePattern: /(?:^|\/)(?:tests?|__tests__)(?:\/|\.)|\.(?:test|spec)\./i, contentPattern: /(?:\btest\(|\bit\(|describe\(|node:test|pytest|unittest)/i, reason: "Test source contains executable test declarations.", mechanism: true },
  ...
  { id: "cli", name: "Command-line interface", description: "Provides commands intended to run from a terminal.", contentPattern: /(?:commander|yargs|argparse|cobra\.Command|clap::Parser)/i, reason: "CLI framework or executable entry-point code is present." }
];
...
export async function analyzeRepository(githubUrl: string): Promise<RepositoryIntelligence> {
  ...
  const manifest = collected.files.find((file) => /(^|\/)(package\.json|pyproject\.toml|cargo\.toml|go\.mod)$/i.test(file.path));
  ...
  if (capabilities.length >= 2) {
    ...
    differentiators.push({
      ...
      description: `The repository implements both ${selected[0].description.toLowerCase()} and ${selected[1].description.toLowerCase()}`,
...
function manifestProductName(file?: CollectedFile): string | undefined {
  if (!file || !file.path.endsWith("package.json")) return undefined;
  try {
