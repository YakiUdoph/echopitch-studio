...
export function assemblePitch(context: ProductionContext, mediaPlan: MediaPlanItem[], productions: SceneProduction[], narration: NarrationProduction, artifactReference: string): AssemblyResult {
  const { scenes, duration } = preparePitch(context, mediaPlan, productions, narration);
  const narrationAssets = resolveNarrationAssets(scenes, narration);
  const assembly: FinalAssembly = {
    artifactReference, downloadReference: `${artifactReference}?download=1`, format: "interactive-html", duration, targetDuration: context.manifest.targetDuration,
...
function preparePitch(context: ProductionContext, mediaPlan: MediaPlanItem[], productions: SceneProduction[], narration: NarrationProduction): { scenes: AssembledScene[]; duration: number } {
  const narrationDurations = new Map(narration.segments?.filter((segment) => segment.status === "generated" && segment.durationSeconds && segment.durationSeconds > 0).map((segment) => [segment.sceneId, segment.durationSeconds!]) || []);
  const scenes = context.manifest.scenes.map((scene) => {
    ...
    return {
      id: scene.sceneId, purpose: scene.purpose, narration: scene.narration, duration: narrationDurations.get(scene.sceneId) || scene.duration,
...
function renderPitchHtml(context: ProductionContext, scenes: AssembledScene[], narration: NarrationProduction, duration: number): string {
  ...
  const payload = safeJson({ title: context.manifest.title, repository: context.intelligence.repository.url, scenes: payloadScenes, duration, targetDuration: context.manifest.targetDuration, audio: narrationAssets.legacyAudio });
  ...
  stage.innerHTML=pitch.scenes.map((s,i)=>'<article class="scene" data-i="'+i+'"><div class="visual">'+visual(s)+'</div><div class="copy"><span class="kicker">'+esc(s.id)+' · '+esc(s.purpose)+'</span><h1>'+esc(pitch.title)+'</h1><p>'+esc(s.narration)+'</p><span class="source">'+esc(s.source.replaceAll('-',' '))+'</span></div></article>').join('');const nodes=[...stage.querySelectorAll('.scene')];const legacyAudio=pitch.audio?new Audio(pitch.audio):null;const sceneAudios=pitch.scenes.map(s=>s.audio?new Audio(s.audio):null);const narrationAudios=legacyAudio?[legacyAudio]:sceneAudios.filter(Boolean);let activeAudio=-1,audioToken=0;narrationAudios.forEach(a=>{a.preload='auto';a.load()});sceneAudios.forEach((a,i)=>{if(a){a.addEventListener('ended',()=>diagnostic('ended',i));a.addEventListener('error',()=>playbackFailure(i,new Error('Audio resource failed to load.')))}});function applyAudioTiming(){if(legacyAudio&&Number.isFinite(legacyAudio.duration)&&legacyAudio.duration>0){pitch.duration=legacyAudio.duration}else if(!legacyAudio){pitch.scenes.forEach((scene,i)=>{const audio=sceneAudios[i];if(audio&&Number.isFinite(audio.duration)&&audio.duration>0)scene.duration=audio.duration});pitch.duration=pitch.scenes.reduce((sum,scene)=>sum+scene.duration,0)}seek.max=String(pitch.duration)}
